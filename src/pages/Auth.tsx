...
type Mode = "login" | "signup" | "forgot" | "otp-email" | "otp-totp";
...
const Auth = () => {
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      if (mode === "forgot") {
        ...
        // Check 2FA method
        const { data: twoFAMethod } = await supabase.rpc("check_2fa_by_email", { _email: email });

        if (twoFAMethod === "totp") {
          // Check if user has TOTP factors - need to challenge
  ...
  const handleVerifyTotpOtp = async () => {
    if (otpCode.length < 6) return;
    setLoading(true);
    try {
      const { data: factors } = await supabase.auth.mfa.listFactors();
      const totpFactor = factors?.totp?.[0];
      if (!totpFactor) throw new Error("No authenticator factor found");
  ...
  if (mode === "otp-email" || mode === "otp-totp") {
    const isTotp = mode === "otp-totp";
    return (
      <div className="min-h-[100dvh] bg-background flex items-center justify-center px-4">
        <div className="w-full max-w-sm space-y-6">
          ...
          <button
            onClick={isTotp ? handleVerifyTotpOtp : handleVerifyEmailOtp}
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground text-center">
            {mode === "forgot"
              ...
              : mode === "signup"
              ? "Sign up to start using Super Copilot"
              : "Sign in to your Super Copilot account"}
          </p>
        </div>
        ...
        {mode !== "forgot" && (
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-border bg-card text-sm font-medium text-foreground hover:bg-accent transition-colors"
            >
